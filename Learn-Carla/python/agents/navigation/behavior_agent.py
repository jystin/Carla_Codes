...
class BehaviorAgent(Agent):
    ...
    def update_information(self, vehicle):
        ...
        """
        self.speed = get_speed(self.vehicle)  # 更新汽车速度
        self.speed_limit = vehicle.get_speed_limit()  # 获得汽车的速度上限，一般为30
        self._local_planner.set_speed(self.speed_limit)  # 用来设置轨迹规划的目标速度，这个速度就是速度上限本身
        # agent里面waypoint_buffer里面最近的一个waypoint的road_option, 用于告诉用户这个点是否涉及到变道。
        ...
        # 后面要使用这个incoming_waypoint和incoming_direction来检测是否即将到达交叉路口
    ...
    def _trace_route(self, start_waypoint, end_waypoint):
        ...
        if self._grp is None:
            ...
            # 主要作用就是提取carla 地图的拓朴结构--> self._topology = self._dao.get_topology()
    ...
    def pedestrian_avoid_manager(self, location, waypoint):
        ...
        # walker_list 是从server里直接拿到的和小车距离十米以内的行人
    ...
    def car_following_manager(self, vehicle, distance, debug=False):
        ...
        """
        跟车模式的核心要点就一个——和前方车辆保持固定的距离。CARLA里实现方式十分简单，如下方代码所示，如果ego和前方汽车的ttc太小，
        ego就会把速度控制到比前方汽车速度还小，如果ttc适中，那就保持和前车一样的速度，如果ttc较大了，那就按原定的目标速度行驶。
        """
    ...
    def run_step(self, debug=False):
        ...
        # 如果汽车所在位置处于交叉口，附近的信号灯为红灯（这个信息是update_information那一步拿到的），
        # 并且汽车本身并没有设置无视红灯的话，那么它就要做一个急刹，尽快停下来。
        ...
        # 和与行人相撞的思路一致, 都是使用了 _bh_is_vehicle_hazard() 这个函数，返回vehicle_state, vehicle, distance三个变量
