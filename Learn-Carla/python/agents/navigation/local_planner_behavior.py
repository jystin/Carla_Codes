...
class LocalPlanner(object):
    ...
    def set_global_plan(self, current_plan, clean=False):
        ...
        for elem in current_plan:
            # 第一件事情就是把上面global planner产生的route全部赋值给waypoints_queue
            #（deque类型数据，carla设置的默认缓存长度为1000）
            # waypoints_queue里面存放的是全局的路径点
        ...
        if clean:
            ...
            for _ in range(self._buffer_size):
                if self.waypoints_queue:
                    # 第二件事情，就是把 buffer_size数量（默认为5）的elements(tuple of waypoint and roadoption)
                    # 从waypoints_queue里弹出，存到短期缓存waypoint_buffer里（也是deque类型数据）用来实现短期的路线。
    ...
    def run_step(self, target_speed=None, debug=False):
        ...
        # waypoints_queue里面没有点了，表示已经到终点了
        if len(self.waypoints_queue) == 0:
            control = carla.VehicleControl()
            # 把control command里面转弯、加速设为0，刹车设为1，意思是车要急刹停下结束这一切了
            control.steer = 0.0
        ...
        #  如果小车还没跑到目的地，那么我们就判断一下装短期全局路线的deque是不是还有目标点存在，
        #  如果没有，那就从waypoint_queue里不停pop目标点出来，装到waypoint_buffer里。
        ...
        # Target waypoint
        # 接着local planner会提取waypoint_buffer里最近的一个点来作为下一个目标点（target_waypoint)
